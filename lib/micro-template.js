			...
			"try { " +
				"with (stash) { this.__ret += '"  +
					string.
						...
						replace(/<%=raw(.+?)%>/g, function (x,_) { return "' + (" + _.replace(/\\x27/g, "'") + ") + '" }).
						replace(/<%=(.+?)%>/g, function (x,_) { return "' + this.escapeHTML(" + _.replace(/\\x27/g, "'") + "+'') + '" }).
						replace(/<%(.+?)%>/g, function (x,_) { return "'; " + _.replace(/\\x27/g, "'") + "; this.__ret += '" }) +
				"'; } return this.__ret;" +
