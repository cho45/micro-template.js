...
const template = function (id, data) {
	...
	if (!me.cache.has(key)) me.cache.set(key, (function () {
		...
		let line = 1;
		// テンプレートの行数をカウント
		const tmplLines = string.split(/\r?\n/).length;
		const mappings = Array(tmplLines).fill('AAAA;').join('');
		const sourceMap = {
			version: 3,
			file: "template.js",
			sources: ["template.ejs"],
			sourcesContent: [string],
			names: [],
			mappings: mappings
		};
		// ソースマップをBase64エンコードしてdata URI化
		const sourceMapDataUri = "data:application/json," + encodeURIComponent(JSON.stringify(sourceMap));
		const body = (
			`try { ` +
